  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Mantenedor de Usuarios & Roles (RBAC)</h1>
      ...
      {feedbackMsg && (
        <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-700/60 text-emerald-800 dark:text-emerald-200 text-xs flex items-center justify-between shadow-xs">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
            <span className="font-semibold">{feedbackMsg}</span>
          </div>
          <button onClick={() => setFeedbackMsg(null)} className="text-emerald-700 dark:text-emerald-300 font-bold hover:underline">
            Cerrar
          </button>
        </div>
      )}
                ...
                filteredUsers.map(user => (
                  <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-[#162744] transition-colors">
                    ...
                    <td className="px-3.5 py-3">
                      ...
                      <div className="text-[11px] text-slate-500 dark:text-slate-400">{user.jobTitle}</div>
                    </td>
